...
void reportTemp() {
	while (run_flag)
	{
		...
		if (SSL_write(SSLstruct, servbuf, strlen(servbuf)) <= 0) {
...
{	
	...
	SSL_CTX* SSLclient = SSL_CTX_new(TLS_client_method());
	SSLstruct = SSL_new(SSLclient);
	
	if (SSL_set_fd(SSLstruct, sockfd) == 0) {
		fprintf(stderr, "Error: Could not create a SSL fd.\n");
		exit(1);
	}
	
	//Initiate handshake
	if (SSL_connect(SSLstruct) != 1) {
		fprintf(stderr, "Error: Could not initiate handshare w/ server.\n");
		exit(1);
	}
	
	//Write ID to server
	if (SSL_write(SSLstruct, id, strlen(id)) <= 0) {
		fprintf(stderr, "Error: could not write to socket.\n");
		exit(1);
	}
	...
	while(1) {
		...
		if (SSL_read(SSLstruct, input, sizeof(char) * 1000) > 0) {
